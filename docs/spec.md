...
<td>v2.17 — Locked. Deviations require version bump.</td>
</tr>
<tr>
...
Decision. Five pieces: four new primary workflows plus one addition to an already-live workflow. (1) Workflow 9 (Auto-Present on Approval) gains a branch that, on every Gate \#2 approval of the pricing deliverable specifically - not only the first - extracts its total figure and PATCHes the associated Deal's amount property, the single automatic source of truth for what a Deal is worth. (2) Generate Deposit Payment Link, triggered by Aaron manually moving a Deal from negotiating to a new awaiting deposit stage, computes 50% of the Deal's amount, creates a Stripe Checkout Session for that exact figure with the Deal ID embedded in its metadata, and emails the client the payment link. (3) Stripe Webhook Receiver, a new public-facing trigger that verifies Stripe's signature on every event and routes by type: checkout.session.completed hands off to (4); charge.dispute.created alerts Aaron directly with no automated action. (4) Record Deposit Payment, a shared, rail-agnostic sub-workflow that checks idempotency against Stripe's event ID, matches the payment to its Deal via the embedded ID, and compares the amount received against the expected 50% figure - a match calls the existing Advance Deal Stage sub-workflow to flip the Deal to real closed-won and then triggers (5); any mismatch (short, over, wrong currency) stops short and raises a Task/Slack alert for Aaron's manual review, the same pattern Gate \#1/#2 already use for content review, rather than guessing. (5) Send Onboarding Kit, triggered once a Deal genuinely reaches closed-won, emails the client the welcome message, a copy of the signed contract, and the payment receipt - all three as direct attachments, not a Google Drive link, matching Workflow 10's own precedent for the same reason: one less permission surface to silently break.

No AI, LLM Chain, or other non-deterministic node appears anywhere in this ADR. Every decision - amount matching, stage advancement, dispute handling - is rule-based, matching Rule 2 (AI prepares, humans decide) and Rule 3 (deterministic where possible) exactly. Detecting a verbal agreement itself is explicitly out of scope: Aaron rejected any AI-based call recording or transcription for the negotiation call, matching his existing boundary around the discovery call itself. Moving the Deal to awaiting deposit is Aaron's own deliberate action and the sole entry point into this automation.
...
Chargebacks are watched, not assumed away. The pipeline's own no-refunds-once-signed policy stops Aaron from voluntarily reversing a payment, but it does not stop a client's card issuer from filing a dispute directly with the bank. The Stripe Webhook Receiver listens for charge.dispute.created specifically so a disputed deposit on an already closed-won, already-onboarding Deal surfaces to Aaron immediately, matching this pipeline's standing fail-loudly convention rather than introducing a new silent-failure mode. Confirmed by Aaron (2026-07-24).
...
Stripe only for now, not exclusively forever. Aaron currently also uses PayPal and Wise, but Stripe is the only rail this ADR builds against: its Checkout product generates the bill and fires the payment webhook in one product, unlike Wise (webhook-only via its Platform API, needing a separate bill/payment-request mechanism) or PayPal (dropped from consideration). Record Deposit Payment is deliberately built rail-agnostic so a future Wise trigger workflow can feed the same shared sub-workflow without a rebuild, matching this pipeline's existing shared-sub-workflow precedent (Resolve Prospect, Advance Deal Stage, Provision Discovery Recording Dropzone). Confirmed by Aaron (2026-07-24).

Currency: USD only. Aaron invoices exclusively in USD; no currency-aware comparison logic exists in this ADR.

A stale payment link after a late pricing change is an accepted risk, not something this build guards against. In practice, awaiting deposit is only entered after verbal agreement, meaning the price should already be locked before any Checkout Session is created. Aaron accepted this as a rare, manually-recoverable edge case. Confirmed by Aaron (2026-07-24).
...
No refunds. Once a contract is signed, the deposit is held until the engagement's goals are accomplished - a deliberate business rule, not a technical default. The chargeback-alerting decision above exists precisely because that policy can be bypassed by the client's own bank even though Aaron will not voluntarily reverse a payment himself.

Required verification before implementation. (1) Confirm the exact type_payload field that holds the pricing deliverable's computed total (Prompt Library §6/B6) so Workflow 9's new sync step reads the right field. (2) Confirm HubSpot's Deal amount property accepts a plain decimal write via the API the same way every other Deal-stage PATCH in this pipeline already works. (3) Aaron creates the new awaiting deposit HubSpot Deal stage before any build begins, choosing its internal ID deliberately - per this project's own established lesson (Dragon-Dance 117), a stage's internal ID is permanent once created; only its display label can be changed later.

Schema / capability impact. One new HubSpot Deal pipeline stage (awaiting deposit, positioned between negotiating and won - Aaron's to create, per the verification note above). New HubSpot Deal properties, exact list finalized at build time under this project's standing HubSpot config-change gate: at minimum a way to record the manual-override lock from the decision above, and likely the expected and received deposit figures for audit. New environment variables: STRIPE_SECRET_KEY, STRIPE_WEBHOOK_SECRET (§11). No new Postgres schema. §7's “10 workflows total” is unaffected - these are new workflows outside that numbered sequence, the same precedent §7g's Outbound Discovery Bootstrap already established.
...
| STRIPE_SECRET_KEY | VPS .env (n8n). From Stripe's API keys settings. Used to create Checkout Sessions for the deposit payment link (§7i). |
| STRIPE_WEBHOOK_SECRET | VPS .env (n8n). From Stripe's webhook endpoint settings. Verifies incoming webhook signatures before any Deal or payment record is touched (§7i). |
