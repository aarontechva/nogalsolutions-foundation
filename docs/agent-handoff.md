...
- **Before writing to `current-state.md`/`next-task.md`/`progress.md`/`baton-pass.state.json`, check for and redact API keys, tokens, passwords, or other credentials — reference them by name/location instead (e.g. "the OpenRouter API key on the VPS") — even though these files are gitignored and local-only, since that isn't a real security boundary (the folder can still be copied, zipped, or backed up whole).** This is scoped to secrets/credentials specifically, not a blanket PII rule — real prospect names/emails/record IDs are legitimate, load-bearing parts of this project's verification discipline (see every "Last Verified Against" entry) and stay exactly as specific as they've always been. (Adopted 2026-07-21 from `mattpocock/skills`' `handoff` skill, Aaron approved.)
- **When a fact, decision, or verification result already has a citable source — a numbered `progress.md` entry, a git commit hash, a live n8n execution ID, a Spec section — cite it instead of re-narrating it in prose.** Restate only what's needed to make the current entry self-contained for its own specific point, not to re-explain background a reader can already look up. `progress.md` has grown past 3000 lines partly from re-narrated context that already existed in an earlier entry. (Adopted 2026-07-21 from `mattpocock/skills`' `handoff` skill, Aaron approved.)
- **`current-state.md` has no baton-pass move that owns its refresh — `save-state`'s own template explicitly excludes it (it only touches `progress.md`/`next-task.md`/`baton-pass.state.json`), so it can silently drift for multiple entries, including across a cross-tool handoff, while `next-task.md`/`progress.md` stay perfectly current.** Every `foresight` receive-check must explicitly diff `current-state.md`'s tail against `progress.md`'s/`next-task.md`'s latest entries, not just read and trust `current-state.md` on its own — and correct it in the same pass if they've diverged. Doubly important right after a cross-tool handoff (Codex ↔ Claude Code CLI). (Dragon-Dance 139, `docs/progress.md` — a Codex session correctly advanced `next-task.md`/`progress.md` through Save-State 135 → Baton-Pass 138 [website case study shipped, Outbound Discovery Bootstrap designed, Job Scraper diagnosed] while `current-state.md` sat frozen at Progress 131 with nothing in the written record surfacing the gap until a receiving agent's `foresight` check caught it directly.) **This recurred a second time (Hindsight 150) — same file, same failure shape, but this time the gap opened during Claude's own same-tool session resumption after a `save-state` pause, not only across a Codex↔Claude handoff.** Resuming a paused conversation doesn't obviously feel like a "receive" moment, so the foresight-diff habit above gets silently skipped exactly when it's needed just as much as at a real handoff. Treat **any** resumption after `state: paused` — same agent or not — as a foresight-diff trigger, not just cross-tool handoffs specifically. (Dragon-Dance 151, `docs/progress.md` — `current-state.md` missed the entire Job Post Analyzer/§7h arc, Progress 143–149, because nobody ran the diff on resuming from Save-State 148.)
- **Any open "awaiting first real trigger" or "recently activated, not yet tested" item for a named n8n workflow gets a direct `n8n_executions` check (action=list, filtered by that workflow's ID) on every `hindsight`/`foresight` pass — independent of whether Aaron has said anything happened.** Waiting for Aaron to report a trigger is not sufficient: a trigger can complete successfully (or fail) with nobody noticing, and the written record has no way to self-correct until someone actually queries live execution history. (Dragon-Dance 153, `docs/progress.md` — §7h's first genuine trigger (BW3 execution `529`) sat completed and fully untraced for ~11 hours, and Job Post Analyzer's only-ever failure execution (`526`, a broken failure-notification path) sat live and undiscovered for almost a day, both found only because Hindsight 152 queried `n8n_executions` directly instead of trusting the written record's silence.)
